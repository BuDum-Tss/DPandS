...
import org.example.schema.PersonType;
...
public class Main {
  ...
  public static void main(String[] args) {
    try (InputStream stream = new FileInputStream("src/main/resources/org/example/people.xml")) {
    ...
    try {
      ...
      jc = JAXBContext.newInstance("org.example.schema", classLoader);
      Marshaller writer = jc.createMarshaller();

      SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
      File schemaFile = new File("src/main/resources/org/example/schema.xsd");
      writer.setSchema(schemaFactory.newSchema(schemaFile));
      writer.setProperty(Marshaller.JAXB_FORMATTED_OUTPUT, true);
      writer.marshal(people, new File("src/main/resources/org/example/output.xml"));
