...
public class PeopleParser {
  ...
  public Map<String, PersonInfo> parse(InputStream stream) throws XMLStreamException {
    ...
    read(reader);
    normalize();
    return data.stream().collect(Collectors.toMap(i -> i.id, i -> i));
  }
  ...
  private void normalize() {
    ...
    currentData = temp_records;
    temp_records = new ArrayList<>();

    combinePersonInfoWithoutNamesakes(currentData, id_records, temp_records);

    currentData = temp_records;
    temp_records = new ArrayList<>();

    combinePersonInfoBySiblings(currentData, id_records, temp_records);
    ...
    data = new ArrayList<>(id_records.values());
  }

  private void combinePersonInfoBySiblings(List<PersonInfo> currentData, HashMap<String, PersonInfo> id_records,
      ArrayList<PersonInfo> temp_records) {
    for (PersonInfo person : currentData) {
  ...
  private void combinePersonInfoWithoutNamesakes(List<PersonInfo> currentData, HashMap<String, PersonInfo> id_records,
      ArrayList<PersonInfo> temp_records) {
    for (PersonInfo person : currentData) {
      List<PersonInfo> found =  findInRecords(x -> x.firstName.equals(person.firstName) && x.lastName.equals(person.lastName),
          id_records.values());
      if (found.size() == 1) {
        PersonInfo foundPerson = found.get(0);
        foundPerson.combine(person);
        id_records.replace(foundPerson.id, foundPerson); // по идее не нужно
      }
      else {
        temp_records.add(person);
      }
  ...
  private void fillUpPersonInfoWithId(HashMap<String, PersonInfo> id_records, ArrayList<PersonInfo> temp_records) {
    for (PersonInfo i : data) {
      if (i.id != null) {
        if (id_records.containsKey(i.id)) {
          id_records.get(i.id).combine(i);
        } else {
